...
async function reopenLastStop(button = null) {
  const remembered = getLastStopSelection();
  if (!remembered?.stopId) return false;
...
async function selectBus(busId, options = {}) {
  ...
  if (options.background && !changedBus && detail.innerHTML) {
    ...
    const shownCamera = document.getElementById("camera-section")?.dataset.cameraId || null;
